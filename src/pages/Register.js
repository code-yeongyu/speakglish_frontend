        ...
        fetch(URL + "/api/signup/", {
            ...
            .then(json => {
                if (!json.account_created) {
                    this.showWarning("이미 존재하는 정보입니다.");
                } else {
                    this.showWarning(
                        "성공적으로 처리되었습니다. 로그인 페이지로 이동하여 로그인하세요."
                    );
                }
            });
    };
    ...
    render() {
        return (
            <div className="register">
                <div className="left">
                    ...
                    <Link to="/login">
                        <Button id="loginButton" animated="fade">
                            ...
                            <Button.Content hidden>로그인</Button.Content>
                        </Button>
                    </Link>
                    <br />
                    <form className="fields" onSubmit={this.handleSubmit}>
                        {this.state.warningVisibility ? (
                            <RegisterWarning text={this.state.warningText} />
                        ) : null}
                        <DjangoCSRFToken />
                        <Input
                ...
                <div className="right">
                    <span>
                        <Icon name="hourglass half" /> 타이머를 설정하세요
                    </span>
                    <span>
                        <Icon name="window maximize" /> 아무 일 이나 하세요
                    </span>
                    <span>
                        <Icon name="exclamation" /> 혼잣말을 해보세요
                    </span>
                    <span>
                        <Icon name="pencil" /> 당신의 컴퓨터가 받아 쓴답니다
                    </span>
                </div>
            </div>
        );
